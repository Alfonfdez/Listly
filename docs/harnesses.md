...
- `tests/screens/` — `HomeScreen.test.tsx` (loading, grid render, empty state, search filter, no-results, FAB/tile navigation, text-size scaling), `ListDetailScreen.test.tsx` (header/progress render, toggle, add at end with position, duplicate/empty rejection, edit modal, delete-after-confirm, empty state), `CreateListScreen.test.tsx` (form render with defaults, create disabled until valid, empty/clamped-name, debounced duplicate, create with selected icon/color + goBack) via `helpers/appStub.ts` (`useApp` mock) + configStub.
