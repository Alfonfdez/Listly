...
### 6. Collection detail
- Header block: tinted icon badge, collection name in its color, `N/total` progress, and an edit pencil → Edit Collection.
...
- Edit Collection also renders an outlined-red *Delete collection* button above *Save*: empty collection → single confirmation; non-empty → the shared chooser (*Move lists to Lists* / *Delete lists too*); deleting returns to the overview.
...
- Edit Collection wires its delete button through it: empty collection → single confirm; non-empty → the chooser.
...
- [x] Deleting a non-empty collection (from Edit Collection) offers *Move lists to Lists* (lists survive as standalone) and *Delete lists too* (lists and their items/photos are removed); deleting an empty collection uses a single confirmation.
...
- [x] List detail and collection detail headers show only search and select toggles (no delete icon); the delete action lives on the Edit List / Edit Collection screens.
- [x] Edit List and Edit Collection render an outlined-red delete button above Save, wired to the confirm/chooser and returning to the overview after deletion.
