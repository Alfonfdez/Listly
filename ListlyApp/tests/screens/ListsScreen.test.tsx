...
import { buildAppMock, setItemsByListId, setLists, resetAppStub } from '../component/helpers/appStub';
...
describe('ListsScreen', () => {
  ...
  it('filters rows by list name and item names', async () => {
    const view = await render(<ListsScreen />);
    await view.findByText('Groceries');
    fireEvent.press(view.getByLabelText('Search lists'));
    const input = await waitFor(() => view.getByPlaceholderText('Search lists and items...'));
    fireEvent.changeText(input, 'briefcase');

    expect(await waitFor(() => view.getByText('Work Tasks'))).toBeTruthy();
